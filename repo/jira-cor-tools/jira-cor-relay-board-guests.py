...
jira_user_api_token='{{ jira_user_api_token_guest }}'
jira_executor_account_id='{{ jira_executor_account_id_guest }}' # Luca Bodini : 5f7d69a13fe07600699920b8
...
print_head()
print_header()
print_title("Jira - COR, External Relay Board Sync")
...
query='project = "COR" AND issuetype!="COR - Epic" AND ("Relay Board" IS NOT EMPTY) AND ("Relay Description" IS NOT EMPTY) AND ("Relay Summary" IS NOT EMPTY) AND ("Severity" IS NOT EMPTY) AND reporter='+jira_executor_account_id
print_item_section('Syncing tickets with Relay Board field set')
mod_count=0
for issue in auth_jira.search_issues(query, maxResults=0):
    print_item_description("Found ticket <a href='{}/browse/{}'>{}</a>, forwarding to: {}".format(jira_endpoint, str(issue), str(issue), str(issue.fields.customfield_11417)))

    if(str(issue.fields.customfield_11419).lower().strip() == "mirror"):
        new_issue_summary = str(issue.fields.summary)
    else:
        new_issue_summary = str(issue.fields.customfield_11419)

    if(str(issue.fields.customfield_11418).lower().strip() == "mirror"):
        new_issue_description = str(issue.fields.description)
    else:
        new_issue_description = str(issue.fields.customfield_11418)



    issue_dict = {
            'reporter': { "id": issue.fields.reporter.accountId },
            'project': str(issue.fields.customfield_11417),
            'summary': new_issue_summary,
            'description': new_issue_description,
            'priority': {'name': str(issue.fields.priority)}
    }

    issue_dict.update(project_defaults[str(issue.fields.customfield_11417)])
    if str(issue.fields.customfield_11417) in ["DCOPS", "TECHOPS"]: issue_dict.pop('reporter') 
    ...
    issue.update(fields={'customfield_11417': None, 'customfield_11419': None, 'customfield_11418': None})
    print_item_list_end()
if (mod_count == 0):
